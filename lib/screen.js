 ...
 */
const crossScreen = (function() {
    let screenStrCss = '<style type="text/css">'+
        '.warn-wp {' +
        'position: fixed;' +
        'z-index: 9999999;' +
        'top: 0;' +
        ...
        '-webkit-animation: touch 2s ease-in infinite;' +
        ...
        '</style>';
    let screenStr ='<div><div class="warn-con">' +
        '<i class="warn-icon"></i>' +
        '<p>为了更好的体验，请使用竖屏浏览</p>' +
        '</div></div>';

    let screenDom = screenStr+screenStrCss;
    ...
    let  detectOrient = ()=> {
        ...
        let el = document.getElementsByClassName('warn-wp');
        if(w == _Width) {
            // 竖屏
            console.log("竖屏");
